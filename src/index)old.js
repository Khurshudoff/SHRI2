...
function prepositioning(scaleX, left = null) {
    ...
    const leftBefore = left !== null ? max(- main_image.getBoundingClientRect().width + img_wrapper.getBoundingClientRect().width +  main_image_left,
        min(main_image.style.transform.split(',')[4], main_image_left))
        : main_image_left;
    ...
    let progress_bar_window_before_style_width = progress_bar_window.style.width;
    ...
    if(left === null){
        ...
        if(progress_bar_window_before_style_width > progress_bar_window.style.width){
            progress_bar_window.style.left = parseFloat(progress_bar_window.style.left.split('p')[0]) +
                ( + ( + parseFloat(progress_bar_window_before_style_width.split('p')[0]) - parseFloat(progress_bar_window.style.width.split('p')[0])) / 2)
                + 'px';
        }
    ...
    function eventsListeners() {
        ...
        function pointermoveFunc(e) {
            ...
            if(pointerSet.size === 2){
                ...
                } else if (previousAngle - angleAfter > .5) {
                    // $(".some_text").text(previousAngle + ' ' + angleAfter);
                    main_image.style.webkitFilter = 'brightness(' +
                        min(1, brightnessBefore - 1 / 180)+
                        ')';
